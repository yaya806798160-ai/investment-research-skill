...
async function renderDashboard(content) {
  const [mkt, pf, alloc, opps, wl, research] = await Promise.all([
    DataSource.getMarket(), DataSource.getPortfolio(), DataSource.getAllocation(),
    DataSource.getOpportunities(), DataSource.getWatchlist(), DataSource.getResearch()
  ]);
  // 市场状态
  ...
  const marketCard = card('Market Overview · ' + esc(mkt.global.regime) +
    ' <span class="' + mkt.global.riskColor + '">风险 ' + esc(mkt.global.riskLevel) + '</span>',
    '<div class="market-tiles">' + tiles + '</div>' +
    '<h3 style="margin-top:16px">Major Events</h3>' + events);
  // 组合概览
  const kpis = [
    { l: '总资产', v: fmtMoney(pf.total), small: false },
    { l: '今日变化', v: pf.today, small: false },
    { l: '收益率', v: pf.return, small: false },
    { l: '风险评分', v: String(pf.riskScore) + '/100', small: true },
    { l: '最大回撤', v: pf.maxDrawdown, small: false }
  ].map(function (k) {
    return '<div class="kpi"><div class="label">' + k.l + '</div>' +
      '<div class="value' + (k.small ? ' small' : '') + '">' + esc(k.v) + '</div></div>';
  }).join('');
  const portfolioCard = card('Portfolio Overview', '<div class="kpis">' + kpis + '</div>');
  // 资产配置
  const legend = alloc.map(function (a) {
    return '<div class="legend-item"><span class="swatch" style="background:' + a.color + '"></span>' +
      '<span>' + esc(a.name) + '</span><span class="pct">' + a.value + '%</span></div>';
  }).join('');
  const allocCard = card('Asset Allocation',
  ...
  const oppCard = card('Opportunity Ranking',
    '<table><thead><tr><th>Asset</th><th>Score</th><th>Reason</th><th>Risk</th></tr></thead><tbody>' + oppRows + '</tbody></table>');
  // 观察池
  ...
  const wlCard = card('Watchlist', wlItems);
  // 最新研究
  ...
  const researchCard = card('Latest Research', latest);

  content.innerHTML =
    '<div class="grid" style="gap:14px">' + marketCard + '</div>' +
    '<div class="grid" style="gap:14px;margin-top:14px">' + portfolioCard + '</div>' +
    '<div class="grid dash-allocation" style="gap:14px;margin-top:14px">' + allocCard + '</div>' +
...
function renderDecision(content) {
  ...
  const run = async function () {
    ...
    resEl.innerHTML =
      ...
      '</div>' +
      '<div class="card" style="margin-top:14px"><h3>风险提示</h3><ul style="padding-left:18px;line-height:1.9">' + riskList + '</ul></div>';
  };
...
async function renderPortfolio(content) {
  ...
  const rows = holdings.map(function (h) {
    ...
    return '<tr><td><strong>' + esc(h.asset) + '</strong></td>' +
      '<td class="num">' + esc(h.pos) + '</td>' +
      '<td>' + esc(h.cat) + '</td>' +
      '<td class="' + rk + '">' + esc(h.risk) + '</td>' +
      '<td>' + esc(h.advice) + '</td></tr>';
  }).join('');
  content.innerHTML =
    '<div class="card"><h3>持仓列表</h3>' +
    '<table><thead><tr><th>标的</th><th>仓位</th><th>分类</th><th>风险暴露</th><th>调整建议</th></tr></thead>' +
    '<tbody>' + rows + '</tbody></table></div>' +
    '<p class="score-meta" style="margin-top:12px">提示：调整建议来自 v2.3 仓位/再平衡引擎的规则输出；实际执行以个人投资政策与人工决策为准。</p>';
}
...
function renderJournal(content) {
  ...
  const form = '<div class="card" style="margin-bottom:14px"><h3>记录投资决策</h3>' +
    '<div class="journal-form">' +
    '<select id="jType"><option value="买入">买入</option><option value="卖出">卖出</option><option value="复盘">复盘</option></select>' +
    '<input id="jAsset" type="text" placeholder="标的（如 NVDA / 组合）" />' +
  ...
  content.innerHTML = form + '<div class="card"><h3>投资日志</h3><div id="jList"></div></div>';
